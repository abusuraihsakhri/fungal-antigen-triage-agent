"""
Command-Line Interface for MycoSentinel: Invasive Fungal Disease Antigen & Biomarker Triager.
"""
import argparse
import csv
import json
...
from .agents import MycologyCoordinator

coordinator = MycologyCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="fungal-antigen-triage-agent", description="MycoSentinel: Invasive Fungal Disease Antigen & Biomarker Triager")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  MYCOSENTINEL: INVASIVE FUNGAL DISEASE ANTIGEN & BIOMARKER TRIAGER")
        print(f"  Domain: Infectious Diseases | Standard: IDSA / SHEA 2024 Antimicrobial Stewardship")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting MycoSentinel: Invasive Fungal Disease Antigen & Biomarker Triager on http://{args.host}:{args.port}")
